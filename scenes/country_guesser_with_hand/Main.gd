...
3. **User Action**: The user indicates to their guessed country on a world map using the tip of his index finger (the whole hand is on screen though).
4. **Map Colors**: The map features countries and continents using topographic representation, with seas in blue.
5. **Verification**: The system focuses on the location of the tip of the user index finer to verify the guess. The Vision API observes the position of the tip of the index finger; the system ensures it aligns with the intended country. Avoid assumptions; verification must be thorough.
6. **Warning**: The tip of the finger is the primary indicator of the pointing location. The system ensures that tip of the index finger is approximately placed on the intended country as interpreted by the Vision API.
...
9. **Image example**: The first 'assistant' message of the discussion is an example of the user pointing at South Africa.
...
*_text_* is mandatory"""
	...
	Global.SYSTEM_IMAGE_URL = ProjectSettings.globalize_path("res://") + "southafrica_hand.png"
