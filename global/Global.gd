...
func check_and_send_to_llm_urls():
	if RECORDED_AUDIO != null and SCREENSHOT != null:
		...
		LlmServer.send_to_llm_server(SYSTEM, "Look closely at the red token position on the world map and determine which country it is close too.", false, SCREENSHOT_URL, SYSTEM_IMAGE_URL)
